...
#[derive(Parser)]
#[clap(global_setting = AppSettings::NextLineHelp, override_usage = "materialized [OPTION]...")]
struct Args {
    // === Special modes. ===
    ...
    #[clap(long, hidden = true, default_value_t)]
    ...
    #[clap(short, long, hidden = true, parse(try_from_str = repr::util::parse_duration), default_value = "5min")]
    ...
    #[clap(long, env = "MZ_TIMESTAMP_FREQUENCY", hidden = true, parse(try_from_str =repr::util::parse_duration), value_name = "DURATION", default_value = "1s")]
    timestamp_frequency: Duration,
    /// Default frequency with which to scrape prometheus metrics
    #[clap(long, env = "MZ_METRICS_SCRAPING_INTERVAL", hidden = true, parse(try_from_str = parse_optional_duration), value_name = "DURATION", default_value = "30s")]
    ...
    #[clap(long, env = "MZ_SYMBIOSIS", hidden = true)]
    ...
    #[clap(long, env = "MZ_TELEMETRY_DOMAIN", hidden = true)]
    telemetry_domain: Option<String>,
    /// The interval at which to report telemetry data.
    #[clap(long, env = "MZ_TELEMETRY_INTERVAL", parse(try_from_str = repr::util::parse_duration), hidden = true)]
